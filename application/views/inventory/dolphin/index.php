<?php
	...
	$apply_search = $_POST['apply_search'];
	...
	wp_enqueue_script(
		'dealertrend-inventory-theme-dolphin',
	...
	if ( empty( $remove_responsive ) ) {
		// Responsive
		wp_enqueue_style(
			'dealertrend-inventory-dolphin-responsive',
	...
	if ( !isset($setprintpage) ) {
		...
		wp_enqueue_style(
			...
			$this->plugin_information[ 'PluginURL' ] . '/application/views/inventory/dolphin/css/dealertrend-inventory-print.css' ,
			false ,
			'0.5'
		);
	}
		...
		if( in_array( $key , $put_in_trail ) ) {
			if( ! empty( $wp_rewrite->rules ) ) {
				...
				if( $key == 'city' || $key == 'state' | $key == 'vin' ) {
					$breadcrumbs .= '<span>&gt;</span><a href="">' . ucfirst( urldecode( $value ) ) . '</a>';
				} else {
