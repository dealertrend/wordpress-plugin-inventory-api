<?php
	if ( !empty( $inventory_sims ) ) {
		$sim_value = '<div class="eagle-content-sidebar-wrapper">';
		$sim_value .= '<div id="eagle-similar-vehicles">';
		$sim_value .= '<div id="eagle-similar-title">Similar Vehicles</div>';
		$sim_value .= '<div id="eagle-similar-items">';
		$sim_counter = 0;
		foreach( $inventory_sims as $inventory_sim):
			...
			if( $sim_on_sale && $sim_sale_price > 0 ) {
				if( $sim_incentive_price > 0 ) {
					$sim_main_price = '<div class="eagle-similar-price">Price $' . number_format( $sim_sale_price - $sim_incentive_price , 0 , '.' , ',' ) . '</div>';
				} else {
					$sim_main_price = '<div class="eagle-similar-price">Price $' . number_format( $sim_sale_price , 0 , '.' , ',' ) . '</div>';
				}
			} else {
				if( $sim_asking_price > 0 ) {
					if( $sim_incentive_price > 0 ) {
						$sim_main_price = '<div class="eagle-similar-price">Price $' . number_format( $sim_asking_price - $sim_incentive_price , 0 , '.' , ',' ) . '</div>';
					} else {
						$sim_main_price = '<div class="eagle-similar-price">Price $' . number_format( $sim_asking_price , 0 , '.' , ',' ) . '</div>';
					}
				} else {
					$sim_main_price = '<div class="eagle-similar-price">' . $sim_default_price_text . '</div>';
				}
			}
			...
			if ( $vin != $sim_vin && $sim_counter < 3 ) {
				...
				// Similar Headline
				$sim_value .= '<div class="eagle-similar-headline">';
				$sim_value .= '<span class="eagle-similar-saleclass">' . $sim_saleclass . '</span>';
				...
				$sim_value .= '</a></div></div>';
				// Similar Info
				$sim_value .= '<div class="eagle-similar-column-right">';
				...
				$sim_value .= '</div>';
				$sim_value .= '<div class="eagle-similar-price">';
